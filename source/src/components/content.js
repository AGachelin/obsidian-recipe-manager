...
export class Content {
    ...
    render(view, internal, container) {
        ...
        if (!view) {
            const inputWrapper = container.createEl('div', { cls: `${UI_CLASSES.INPUT_FIELD} ${UI_CLASSES.CONTENT_INPUT}` });
            internal.renderMarkdown(this.edit, inputWrapper, this.path);
        } else {
            const viewWrapper = container.createEl('div', { cls: `${UI_CLASSES.VIEW_FIELD} ${UI_CLASSES.CONTENT_VIEW}` });

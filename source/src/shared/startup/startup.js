const { PATHS } = await engine.importJs('source/src/shared/constants/paths.js');
const { intializeMathUnits } = await engine.importJs('source/src/shared/startup/math-units.js');
const { CUSTOM_UNITS } = await engine.importJs('source/src/shared/constants/custom_units.js');

engine.app.PATHS = PATHS;
...
const mathUnits = intializeMathUnits(mb);
mb.mathJSImport(mathUnits);
